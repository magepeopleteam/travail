...
/**
 * Merge template-part $args with design defaults.
 *
 * Empty strings from Elementor (a control left blank) are treated as
 * "use the design default" so an unedited widget still matches the
 * reference layout pixel-for-pixel. False/0/arrays are kept as-is.
 *
 * @param mixed $args     Template $args (may be unset/null).
 * @param array $defaults Design defaults.
 * @return array
 */
function travail_section_args( $args, $defaults ) {
	if ( ! is_array( $args ) ) {
		$args = array();
	}
...
/**
 * Whether "Travello" is the active homepage design on the front page —
 * whether it's rendering via the old hook-based homepage (see
 * inc/homepage-travello.php) or a real Elementor page generated by
 * Travail → Homepages. This is what asset-loading/body-class code
 * should gate on (travello.css/.js, the 'travail-travello-home' body
 * class, which hero image to preload) — it answers "is Travello showing
 * right now", not "which render path is producing it".
 *
 * A generated Elementor page's own `_travail_homepage_design` post meta
 * is checked FIRST and, when present, is authoritative — regardless of
 * the legacy Customizer setting read as the fallback below, so an
 * Elementor "Travail Classic" page correctly reports false here even if
 * the Customizer was never changed from its 'travello' default (and vice
 * versa). See travail_should_render_travello_hook() for the front page
 * TEMPLATE decision, which is a related but different question — mixing
 * the two up here previously meant an active Elementor Travello page
 * loaded none of its own travello.css/.js (confirmed: this broke the
...
/**
 * Whether front-page.php should render the OLD hook-based Travello
 * homepage (do_action('travail_travello_homepage')) rather than
 * deferring to a real Elementor-generated page's own the_content().
 *
 * A page generated by the Elementor homepage builder (Travail →
 * Homepages, see inc/importer/class-travail-elementor-page-builder.php),
 * once assigned as the front page, is a real, fully-editable Elementor
 * page — front-page.php should render its own the_content() instead of
 * either hook-based homepage variation, regardless of which design that
 * page is or what the legacy Customizer setting says.
 *
 * @return bool
 */
function travail_should_render_travello_hook() {
	if ( ! is_front_page() ) {
		return false;
	}
